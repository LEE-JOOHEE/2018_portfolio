$(function () {
  $("#gallery").each(function () {
    ...
    function initGallery(data) {
      ...
      // 필터 라디오 버튼이 변경되면 필터링을 수행 - input[type="checkbox"]이런식으로 바꾸면 된다.
    ...
    function addItems(filter) {
      ...
      // slicedData의 요소마다 DOM 요소를 생성 - DOM은 트리형태의 뻗어나가는 구조
      $.each(slicedData, function (i, item) {
        var itemHTML =
          ...
          '<b class="title">' +
          item.title +
          "</b>" + //<b태그>굵게-이미지위에 타이틀글짜굻게
          '<time class="date" datatime="' +
          item.date +
          '">' +
  ...
  // Resize header - 헤더 사이즈가 넓어졌다 스크롤 내리면 줄어드는 리사이즈 헤더
  $("header").each(function () {
    ...
    $(window).on(
      ...
      $.throttle(1000 / 60, function () {
        ...
        $header.css(styles); //css 'header부분'에 있는거 불러오는 방법
      })
    );
  });
});
